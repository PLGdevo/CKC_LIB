...
public:
    ...
    void CKC_publishData_status(const char *data);
...
template <class MQTT>
inline void CKC_MQTT<MQTT>::CKC_publishData_status(const char *data)
{
    char NameTopic[100];
    snprintf(NameTopic, sizeof(NameTopic), "%s%s", CKC_BASE_TOPIC, CKC_PUB_PREFIX_STATUS_TOPIC);
    mqttClient.publish(NameTopic, data);
}
...
inline void CKC_MQTT<MQTT>::begin()
{
    ...
    {
        CKC_LOG_FAIL("MQTT", "FAILED, rc=", mqttClient.state());
    }
}
template <class MQTT>
